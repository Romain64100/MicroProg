...
/******************************** Initialisation ******************************/
  ...
  PORTD = 0;
   ...
   /***** Question 5 - a completer et a commenter *****/

  TRISC.TRISC0 = 0;                // Initialisation des broches RC0 a RC4 en mode output 
  TRISC.TRISC1 = 0;
  TRISC.TRISC2 = 0;                // --> to control lamps L0 to L4
  TRISC.TRISC3 = 0;
  TRISC.TRISC4 = 0;

  TRISA.TRISA0 = 1;                // utilisation de RA0 en mode analogique

  ANSEL = 0;
  ANSEL.ANS0 = 1;                  // utilisation de RA0 en mode analogique
  ANSELH = 0;

  /***** ADC configuration *****/

  ADCON0.ADCS1 = 1;                // configuration de la periode d'horloge du ADC
  ADCON0.ADCS0 = 0;                // ici configurer a Fosc/32

  ADCON1.VCFG1 = 0;                // utilisation de Vss comme Vref- pour l'ADC
  ADCON1.VCFG0 = 0;                // utilisation de VDD comme Vref+ pour l'ADC

  ADCON0.CHS3 = 0;                 // Selection du pin AN0
  ADCON0.CHS2 = 0;
  ADCON0.CHS1 = 0;
  ADCON0.CHS0 = 0;
  ADCON1.ADFM = 1;                 // Right justified resulat

  ADCON0.ADON = 1;                 // ADC enable
...
unsigned ReadADC() {
  ...
  ADCON0.GO = 1;                   // � commenter
  while(ADCON0.GO);                // � commenter

  result = ADRESH*256 + ADRESL;    // � commenter
